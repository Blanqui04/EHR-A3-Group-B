...
\begin{itemize}
    \item \textbf{Sensitivity (60.4\%):} The model correctly identifies approximately 60\% of patients who will die in-hospital, enabling targeted interventions for high-risk cases.
    ...
    \item \textbf{PPV (40.6\%):} When the model predicts mortality, it is correct les than half of the time.
    \item \textbf{NPV (74.9\%):} The model provides reasonable confidence when predicting survival. 
\end{itemize}

These performance characteristics suggest that the model has limited clinical utility for individual risk stratification but confirms that demographic factors alone are insufficient for accurate mortality prediction.
